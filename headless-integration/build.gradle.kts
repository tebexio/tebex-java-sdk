// Live integration tests for the generated Headless API client.
//
// Unlike :tbx's unit tests (which stub the API in-JVM), these call the real
// https://headless.tebex.io against a real store, so they catch the class of
// bug a stub cannot: the contract (apis/headless-api.yaml) disagreeing with
// what the API actually returns. They exercise the client exactly as a
// consumer does — through io.tebex.http.HeadlessApi.
//
// Opt-in: every test is skipped unless TEBEX_IT_PUBLIC_TOKEN is set, so
// `./gradlew build` and CI stay offline. See LiveStore for the other
// (optional) variables. Run with:
//   TEBEX_IT_PUBLIC_TOKEN=... ./gradlew :headless-integration:test
//
...
dependencies {
    // :tbx for the HeadlessApi facade and its response corrections;
    // :headless-api for the generated types, which :tbx only compiles against
    // (they are merged into its jar rather than exposed as a dependency).
...
val reportContractDrift = tasks.register("reportContractDrift") {
    ...
    doLast {
        ...
        logger.warn("WARNING: the API returned ${grouped.size} propert${if (grouped.size == 1) "y" else "ies"} " +
            "not defined in apis/headless-api.yaml. The client keeps them in additionalProperties, " +
...
tasks.test {
    ...
    // LiveStore names the contract schema an undeclared property belongs to.
    systemProperty("tebex.contract", rootProject.file("apis/headless-api.yaml").absolutePath)
    inputs.property("liveTokenPresent", System.getenv("TEBEX_IT_PUBLIC_TOKEN") != null)
    ...
    // LiveStore.checkContract appends each property the API returned but the
